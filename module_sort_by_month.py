...
class ImageSorterMonthApp:
    def __init__(self, master):
        self.master = master
        # self.master.title("Renaming by Month Module")
    ...
    def create_widgets(self):
        ...
        # Label at the top
        top_label = tk.Label(main_frame, text="Renaming by Month Module", font=("Helvetica", 16))
        ...
        # Description label
        description_label = tk.Label(main_frame, text="Description: This module organizes images by the month of creation.")
    ...
    def get_creation_month(self, file_path):
        try:
            # Try to get the creation month from EXIF data
    ...
    def organize_images(self):
        ...
        for filename in os.listdir(input_folder):
            if any(filename.lower().endswith(ext) for ext in valid_extensions):
                ...
                creation_month = self.get_creation_month(image_path)

                if creation_month is not None:
                    month_folder = os.path.join(output_folder, f"{creation_month:02d}")
